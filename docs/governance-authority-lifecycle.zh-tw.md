...
現有 approval method 可以直接 restore bound rejected current revision；這是 implementation behavior，不是 fresh review 的證明。Approval method 會拒絕 blocked current revision。Current-target enforcement 已實作；在 stronger workflow 實作前，production policy 仍應要求 fresh decision evidence，且不得聲稱 database 已強制 rebinding、expiry、quorum 或 evidence freshness。

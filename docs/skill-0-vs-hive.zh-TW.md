...
> [!WARNING]
> 此比較基於 Skill-0 儲存庫（<owner>/skill-0）與 Hive 儲存庫（bryanadenhq/hive）截至 2026-02 的內容。專案功能與定位可能隨時間變更。Hive 由 Aden（Y Combinator）支持且處於積極開發中；路線圖項目（JS/TS SDK、雲端部署、評估系統）可能在本文件更新前上線。

## 來源

- Skill-0 儲存庫: https://github.com/<owner>/skill-0

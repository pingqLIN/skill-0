...
- 所有 production Dockerfile stages 現在都已 digest-pin。使用 pinned images 的第二次 isolated Compose rehearsal 已通過 build、health、production doctor、governed dry-run、deterministic Evidence、three-store backup/restore、restart persistence 與 zero-resource cleanup。
- Regression：451 個 Python test、34 個 Web test 通過；frontend production build 與 Python compile check 通過。
- Follow-up pinned-image regression：504 個 Python/API tests、36 個 frontend tests 通過；frontend lint/build 與 schema validation 196/196 通過。
...
2. **可變動 supply-chain reference — 部分解決的 Warning。** API、Dashboard、Web build 與 Web runtime stages 都已 digest-pin；GitHub Actions 仍使用 mutable tags。應在獨立 hardening batch pin Action commit SHA，再用 update bot 避免靜默過期。

...
Final API image 仍含 Debian Bookworm essential Perl runtime 的 **1 個 Critical 與 2 個 High** finding，Bookworm 目前都沒有 fixed version。處置方式是明確禁止 production，而不是接受風險：此 image 不具備 production、release、deploy 或 public exposure clearance。Dashboard 與 Web image 的 CVE 狀態仍為 **UNKNOWN**，因為其 Scout scan 未完成。
...
## 剩餘 Warnings／Unknowns

1. **Dashboard／Web container CVE inventory — UNKNOWN，production blocker。** 兩者的 Scout attempt 未完成。成功 build 與 package-manager audit 都不能取代 OS／package-level scan。Docker 說明一次性 CLI analysis 不會保存 image 資料，且 `local://` 可禁止 registry fallback：[Docker Scout image analysis](https://docs.docker.com/scout/explore/analysis/) 與 [`docker scout cves`](https://docs.docker.com/reference/cli/docker/scout/cves/)。
2. **可變動 supply-chain reference — Warning。** `Dockerfile.dashboard`、`Dockerfile.web` 的 Node build stage，以及 GitHub Actions 使用 mutable tag。應在獨立 hardening batch pin image digest 與 Action commit SHA，再用 update bot 避免靜默過期。
3. **Legacy lock 不完整 — Warning。** `requirements.lock` 未被 CI 或 container 使用，也不是具有 hash 的完整 transitive lock。現在僅保留為已標示的 legacy snapshot；後續應改成按環境拆分、hash-verified lock，或依 repository 的 recoverable deletion workflow 移除。
4. **Model source boundary — Warning。** `SkillEmbedder` 優先使用 local cache，但仍可 fallback 至遠端下載。Production 應要求已批准且有 digest 的 local model artifact，不應接受任意 `SKILL0_EMBEDDING_MODEL` 值。

...
本 Gate A 文件定義並記錄 current-target enforcement 的 compatibility-only implementation，同時保留 fresh reapproval 尚未決定的事項。它不改 authority tuple、Runtime admission、Asset type、Dashboard design，也不允許 physical database migration。
...
Implementation 讓 `approve_skill()`、`reject_skill()`、`record_security_scan()`、`record_equivalence_test()` 共用此規則。Evidence writer 以 stable `GovernanceTargetError` 失敗；boolean approve/reject caller 維持既有 `False` contract。Dashboard worker 把 job 保存的 `target_revision_id` 傳到 scan/test write。若 target 在 execution 前或期間被 superseded，job 以 non-retriable `STALE_TARGET_REVISION` 失敗，不得 retarget、更新 projection 或產生 successful evidence；operator 必須為 current revision 建立新 job。

省略 `revision_id` 的 synchronous caller 維持 current behavior；evidence write 使用 stable domain error，action job 使用 stable structured error code。Gate A 不新增 HTTP request field，也不 redesign UI。

### Candidate files

- `tools/governance_db.py`：approve/reject/scan/equivalence 的 current-target resolver。
- `skill-0-dashboard/apps/api/services/governance.py`：傳遞 scan/test captured target，stale job 不 retarget。
...
2. Queued scan/test 在 execution 前被 superseded，必須以 `STALE_TARGET_REVISION` 失敗；不得 retry 或 retarget。

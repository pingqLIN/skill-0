...
- [`../production-security-policy.md`](../production-security-policy.md) 已提升至 policy `1.2.0`；上述兩個控制不再列為 known unenforced gaps。
...
這不等於 production clearance。Dependency/image review 仍為 `PRODUCTION_NO_GO_PENDING_BASE_CVE_FIX`，Dashboard/Web image vulnerability state 仍是 `UNKNOWN`；external TLS、network、secret-manager、encrypted-backup 與 monitoring controls 仍需 operator evidence。

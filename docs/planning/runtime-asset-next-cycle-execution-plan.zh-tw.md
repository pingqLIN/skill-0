...
這不等於 production clearance。Dependency/image review 仍為 `PRODUCTION_NO_GO_PENDING_BASE_CVE_FIX`：`2026-07-21` Web-only refresh 現已在所有 severity 掃描為零，但 API 與 Dashboard 因 Bookworm 尚無 Perl 修正版，仍各有 1 Critical／2 High。External TLS、network、secret-manager、encrypted-backup 與 monitoring controls 也仍需 operator evidence。

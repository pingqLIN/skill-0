...
**狀態：** `2026-07-20` 已完成本機 source/contract hardening；isolated Compose rehearsal 受本機 deployment prerequisites 阻擋。
...
- [`../production-security-policy.md`](../production-security-policy.md) 已提升至 policy `1.1.0`；上述兩個控制不再列為 known unenforced gaps。

### Validation evidence

- Focused production/API/security regression：`96 passed`。
- Full Python/API regression：`497 passed, 62 warnings`。
- PowerShell Compose rehearsal 已成功 parse，但未執行。
...
### 被阻擋的 deployment evidence

請求的 live isolated rehearsal 為 `BLOCKED`，不是 passed：Docker client `29.6.1` 已安裝，但本機 daemon pipe 不可用；且刻意沒有讀取或建立真實 production `CORS_ORIGINS`。沒有 container、route、secret file、volume、external service 被啟動或修改。僅在本機 Docker daemon 與 operator 提供的 staging configuration 可用後，才能重跑既有 rehearsal；在此之前不得宣稱具備 production readiness。

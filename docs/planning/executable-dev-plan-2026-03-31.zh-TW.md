...
4. **CI 文件漂移防護仍需擴充**
   - 現況：`docs-status`、shared-doc source-set drift gate、`skill-0-GUI` mirrored docs drift check、以及 repo-local ownership/provenance boundary marker enforcement 已落地；剩餘工作主要是是否要在 `skill-0-GUI` 自己的 workflow 內也獨立執行同一套契約檢查
...
### 2.3 最新驗證事實（截至 2026-04-27）

以下事項已由主代理在本地重新驗證，可作為本計畫的最新證據基線：

1. `parsed/` 目前共有 `196` 個 checked-in JSON。  
2. `converted-skills/` 目前共有 `164` 個目錄。  
3. Python 測試收集數量為 `221`，最近一次完整回歸結果為 `221 passed, 61 warnings`。
4. Dashboard web 測試結果為 `26 passed`。  
5. Dashboard web production build 已成功完成。  
6. `python tools/validate_skill_schema.py parsed` 結果為 `196 passed, 0 failed`。  
7. `check_doc_status_markers.py`、`check_shared_docs.py`、`check_shared_docs_mirror.py` 皆已通過。  
8. Dashboard web dependency safe bump 後，local `npm audit --json` 結果為 `0 vulnerabilities`。
...
1. `CP-04` 已部分落地：  
   ...
   - 已新增 cross-repo `tools/check_shared_docs_mirror.py`，並在 CI 驗證 `skill-0-GUI/docs/shared/` mirrored copies 不漂移
   - 已新增 shared-doc ownership / provenance boundary marker 檢查，防止 source-of-truth 與 mirrored-copy 邊界被寫模糊
   - 後續主要剩下 `skill-0-GUI` 是否也要在自己的 workflow 內獨立執行同類 gate
...
3. `CP-06` 已完成 gating 收斂：  
   - `plan-20-skills.md` 已明確改為 gated backlog，而非直接執行單
   - 已補上啟動條件：governance/runtime/docs baseline、full regression 綠燈、以及候選 skill 來源重新盤點
   - 後續只在 gate 成立後才重啟實際擴展批次
...
6. `CP-07` 已於 `2026-04-27` 完成本輪基線驗證：
   - `python tools/validate_skill_schema.py parsed` → `196 passed, 0 failed`
   - `python tools/check_doc_status_markers.py` / `python tools/check_shared_docs.py` / `python tools/check_shared_docs_mirror.py --gui-root <skill-0-gui-root> --require-gui-root` 全數 passed
   - `python -m pytest tests skill-0-dashboard/apps/api/tests -q` → `221 passed, 61 warnings`
   - `cd skill-0-dashboard/apps/web && npm audit --json` → `0 vulnerabilities`
   - `cd skill-0-dashboard/apps/web && npm test -- --run` → `26 passed`
   - `cd skill-0-dashboard/apps/web && npm run build` 已成功完成

7. `CP-01` 已於 `2026-04-02` 完成收斂：  
...
10. `CP-02` 前端覆蓋已於 `2026-04-03` 再往前推一格：
...
11. `CP-02` durability / restart recovery 已於 `2026-04-03` 落地：
...
12. `CP-02` worker claim discipline 已於 `2026-04-03` 補齊：
...
13. `CP-02` lease / heartbeat 已於 `2026-04-03` 再補一格：
...
1. 將文件權威索引與 README / AGENTS / current reports 納入定期 drift check，避免歷史數字再被誤用。
2. 推進 CP-02 下一階段：補上 job telemetry、取消語義，以及更完整的 lease expiry / heartbeat observability。
3. 擴大 async UI 覆蓋面：評估是否將 batch job 狀態延伸到更多治理入口，而不重複造輪子。

### Sprint S2（2-4 天）

1. 視需要延伸 CP-04 到 `skill-0-GUI` 自己的 workflow，讓 mirrored docs gate 不只由 `skill-0` 單邊承擔。  
2. 補齊 CP-03 長尾 hardening：觀測性、部署假設與回歸防護。  
3. 在 gate 成立後才重啟 CP-06 的實際 20-skills 擴展批次，而不是提前開工。  

### Sprint S3（1-2 天）

1. 視需要重新跑 CP-07，作為後續 hardening / 外審前的回歸基線。  
2. 完成 CP-08：輸出外部審查包。  

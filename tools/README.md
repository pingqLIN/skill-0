...
以 read-only source Index 建立 disposable vector snapshot，並對三個預先登記的
FTS5 profile（baseline、`detail=none`、`detail=none,columnsize=0`）比較
sqlite-vec、FTS5 BM25 與 RRF hybrid。執行前後會驗證 source Index SHA、size 與
mtime 不變；工具不修改 production schema 或 operator DB。

```powershell
.\.venv\Scripts\python.exe tools\runtime_asset_search_benchmark.py `
  --source-index skills.db `
  --parsed-dir parsed `
  --queries benchmarks\runtime-asset-search-queries-v2.json `

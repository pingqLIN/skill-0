...
1. **不取代原 Skill，先做旁路解析**：保留既有 `SKILL.md`/工具定義，先用 Skill-0 轉成 JSON 做分析與檢索。
2. **建立雙向對照**：將原始段落對應到 `actions/rules/directives`，作為文件與測試的共同語彙。
3. **逐步上線**：先用在「搜尋、治理、審查」流程，再視需要回寫到開發流程（例如模板/驗證）。

### 可落地的應用情境

- **Skill 盤點與去重**：用語義搜尋找出重複能力或可合併的 Skill。
- **治理審查**：用結構化欄位（規則、限制、失敗模式）做一致性檢查。
- **維運交接**：將隱性知識拆成可查詢元素，降低只靠作者記憶的風險。

### 使用上的優缺點

| 面向 | 優點 | 可能缺點/成本 |
|------|------|---------------|
| 相容性 | 可先旁路導入，不需一次重寫全部 Skill | 初期需要維護原文與結構化結果兩份資料 |
| 開發效率 | 結構化後更容易做檢索、比對、模板化 | 初次分解與標註有學習與工時成本 |
| 品質治理 | 規則與指示可被驗證、審查流程可標準化 | 受限於分解品質，錯誤標註會影響後續分析 |
| 擴展性 | 可串接 API、Dashboard、向量搜尋做跨 Skill 管理 | 若 Schema 升版，需規劃相容策略與遷移 |
...
├── parsed/                            # 已解析的 skill 範例（195 個已提交 JSON）
├── tools/                             # 分析與治理工具
├── scripts/                           # 維護腳本
├── tests/                             # 測試套件與 fixtures
...
# 建立 repo-local Python 虛擬環境
python3 -m venv .venv
source .venv/bin/activate

# 安裝 core API + dashboard API + 測試依賴
...
# 自然語言搜尋
.venv/bin/python -m vector_db.search --db skills.db search "PDF 文件處理"
...
```bash
$ .venv/bin/python -m vector_db.search search "創意設計視覺藝術"
...
目前索引與搜尋效能會隨硬體、模型快取狀態，以及 embedding model 是否已下載而變動。若要取得你這台機器的最新數值，請在本地重新執行 `.venv/bin/python -m vector_db.search --db skills.db --parsed-dir parsed index`。
...
- **新功能**: 向量嵌入語義搜尋
  ...
  - K-Means 聚類 skill 分群
  - CLI 工具: `.venv/bin/python -m vector_db.search`

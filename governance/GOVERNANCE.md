...
Skill-0 的治理系統不以「先列出有哪些 skill，再自下而上拼裝流程」為核心思路。

相反地，它比較接近 top-down、intent-driven 的管理方式：

1. 先由目標、任務階段、治理限制與上下文決定目前要執行的治理路徑
2. 再向下調用 intake、scan、fidelity、registry、audit 等能力

因此，skill discovery、metadata 與資料庫紀錄雖然重要，但它們在治理架構中屬於被調度的能力層，不是最高層的決策來源。

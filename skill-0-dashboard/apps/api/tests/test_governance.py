...
class TestActionReadiness:
    def test_readiness_all_ok(self, client, mock_service, auth_header):
        resp = client.get("/api/skills/skill_001/action-readiness", headers=auth_header)
    ...
    def test_readiness_skill_not_found(self, client, mock_service, auth_header):
        ...
        resp = client.get("/api/skills/nonexistent/action-readiness", headers=auth_header)
...
class TestScanAction:
    def test_scan_single_skill_success(self, client, mock_service, auth_header):
        resp = client.post("/api/skills/scan?skill_id=skill_001", headers=auth_header)
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_scan_source_path_missing(self, client, mock_service, auth_header):
        mock_service.run_scan.return_value = {
            ...
            "error_message": "Source path does not exist: /missing",
    ...
    def test_scan_skill_not_found_returns_error_code(self, client, mock_service, auth_header):
        mock_service.run_scan.return_value = {
            "status": "failed",
            "skill_id": "ghost",
            "processed": 0,
            ...
            "error_message": "Skill not found: ghost",
            "hint": "Verify the skill_id is correct.",
        }
        resp = client.post("/api/skills/scan?skill_id=ghost", headers=auth_header)
        assert resp.status_code == 200
        data = resp.json()
        assert data["error_code"] == "PATH_NOT_FOUND"

    def test_scan_empty_target_noop(self, client, mock_service, auth_header):
        """POST /api/skills/scan without skill_id calls batch which returns noop when no skills."""
    ...
    def test_scan_batch_partial_success(self, client, mock_service, auth_header):
        mock_service.run_scan_batch.return_value = {
            "status": "partial",
            "processed": 3,
            "results": [
                {"skill_id": "s1", "status": "success"},
                {"skill_id": "s2", "status": "failed", "error_code": "SOURCE_PATH_MISSING"},
                {"skill_id": "s3", "status": "success"},
            ],
            "error_code": None,
            "error_message": None,
            "hint": None,
...
class TestTestAction:
    def test_test_single_skill_success(self, client, mock_service, auth_header):
        resp = client.post("/api/skills/test?skill_id=skill_001", headers=auth_header)
        assert resp.status_code == 200
        data = resp.json()
        assert data["status"] == "success"
        assert data["processed"] == 1
    ...
    def test_test_installed_path_missing(self, client, mock_service, auth_header):
        mock_service.run_test.return_value = {
            ...
            "error_message": "Installed path does not exist: (not set)",
    ...
    def test_test_empty_target_noop(self, client, mock_service, auth_header):
    ...
    def test_test_batch_partial_success(self, client, mock_service, auth_header):
        mock_service.run_test_batch.return_value = {
            "status": "partial",
            "processed": 2,
            "results": [
                {"skill_id": "s1", "status": "success"},
                {"skill_id": "s2", "status": "failed", "error_code": "INSTALLED_PATH_MISSING"},
            ],
            "error_code": None,
            "error_message": None,
            "hint": None,
...
# ---------------------------------------------------------------------------
# Service unit tests (GovernanceService without DB)
# ---------------------------------------------------------------------------


class TestGovernanceServiceReadiness:
    """Unit tests for get_action_readiness using a patched DB."""

...
class TestActionReadiness:
    def test_readiness_all_ok(self, client, mock_service):
        resp = client.get("/api/skills/skill_001/action-readiness", headers=_auth_headers())
    ...
    def test_readiness_skill_not_found(self, client, mock_service):
        ...
        resp = client.get("/api/skills/nonexistent/action-readiness", headers=_auth_headers())
...
class TestScanAction:
    def test_scan_single_skill_success(self, client, mock_service):
        resp = client.post("/api/skills/scan?skill_id=skill_001", headers=_auth_headers())
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_scan_single_skill_failed(self, client, mock_service):
    ...
    def test_scan_batch(self, client, mock_service):
        resp = client.post("/api/skills/scan", headers=_auth_headers())
        assert resp.status_code == 200
        mock_service.run_scan_batch.assert_called_once()

    def test_scan_batch_noop(self, client, mock_service):
        resp = client.post("/api/skills/scan", headers=_auth_headers())
        assert resp.status_code == 200
        data = resp.json()
...
class TestTestAction:
    def test_test_single_skill_success(self, client, mock_service):
        resp = client.post("/api/skills/test?skill_id=skill_001", headers=_auth_headers())
        assert resp.status_code == 200
        data = resp.json()
        assert data["status"] == "success"
        assert data["processed"] == 1
    ...
    def test_test_single_skill_failed(self, client, mock_service):
        mock_service.run_test.return_value = {
            ...
            "error_message": "Installed path not set",
            "hint": "Check installed_path",
        }
        resp = client.post("/api/skills/test?skill_id=skill_001", headers=_auth_headers())
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_test_batch(self, client, mock_service):
        resp = client.post("/api/skills/test", headers=_auth_headers())
        assert resp.status_code == 200
...
class TestGovernanceServiceReadiness:
    """Unit tests for get_action_readiness without mocking the service itself."""
    ...
    def _make_service(self, skill: Optional[MagicMock] = None):

...
# Skip entire module when running in offline mode (CI) since these tests
# require SentenceTransformer model downloads from HuggingFace.
...
class TestAuth:
    ...
    def test_login_success(self, client, monkeypatch):
        """POST /api/auth/token returns access token with configured credentials"""
    ...
    def test_login_invalid_credentials(self, client, monkeypatch):
        """POST /api/auth/token rejects wrong password"""
        monkeypatch.setenv("API_USERNAME", "admin")
        monkeypatch.setenv("API_PASSWORD", "secret")
    ...
    def test_login_rejected_when_auth_not_configured(self, client, monkeypatch):
        """POST /api/auth/token returns 503 when auth env vars are missing"""
        ...
        assert "Authentication is not configured" in resp.json()["detail"]
